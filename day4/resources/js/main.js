...
// Get your shorts on - this is an array workout!
    ...
    // One line version
    const fifteenHundredBorn = inventors.filter(inventor => (inventor.year <= 1599 && inventor.year >= 1500))
    // const fifteenHundredBorn = inventors.filter(function(inventor){
    //     if(inventor.year <= 1599 && inventor.year >= 1500){
    //         return true;
    //     }
    // })
    console.table(fifteenHundredBorn)
    // Array.prototype.map()
    ...
    const transportation = data.reduce((resultObj, item) => {
        if(!resultObj[item]){
            resultObj[item] = 0;
        }
        resultObj[item]++;
